...
class MoBY(nn.Module):
    def __init__(self,
        ...
        if cfg.LOCAL_RANK == -1:
            self.K = int(self.cfg.DATA.TRAINING_IMAGES * 1. / self.cfg.DATA.BATCH_SIZE) * self.cfg.TRAIN.EPOCHS
            self.k = int(self.cfg.DATA.TRAINING_IMAGES * 1. / self.cfg.DATA.BATCH_SIZE) * self.cfg.TRAIN.START_EPOCH
        else:
