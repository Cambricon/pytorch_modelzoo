...
def P3D199(pretrained=False, modality='RGB', **kwargs):
    ...
    if pretrained == True:
        if modality == 'RGB':
            ckp_root = os.getenv('PYTORCH_TRAIN_CHECKPOINT')
            pretrained_file = os.path.join(ckp_root, 'p3d/p3d_rgb_199.checkpoint.pth.tar')
            if not os.path.isfile(pretrained_file):
                print("ERROR, pretrained model '{}' not exist".format(pretrained_file))
                exit()
        elif modality == 'Flow':
            pretrained_file = 'p3d_flow_199.checkpoint.pth.tar'
        weights = torch.load(pretrained_file)['state_dict']
        print('loading trained model, ', pretrained_file)
