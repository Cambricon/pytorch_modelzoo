...
class PrefetchLoader:
    ...
    def __init__(self,
        ...
        if torch.cuda.is_available():
            ...
            self.mean = torch.tensor([x * 255 for x in mean]).to(ct.mlu_device()).view(1, 3, 1, 1)
            self.std = torch.tensor([x * 255 for x in std]).to(ct.mlu_device()).view(1, 3, 1, 1)
