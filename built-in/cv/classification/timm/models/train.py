...
def main():
    ...
    if args.device == 'gpu':
        args.device_id = 'cuda:0'
    elif args.device == 'mlu':
        args.device_id = 'mlu:0'
    args.world_size = 1
    args.rank = 0  # global rank
    if args.distributed:
        if args.device == 'gpu':
            args.device_id = 'cuda:%d' % args.local_rank
            torch.cuda.set_device(args.local_rank)
            torch.distributed.init_process_group(backend='nccl', init_method='env://')
        elif args.device == 'mlu':
            args.device_id = 'mlu:%d' % args.local_rank
            ct.set_device(args.local_rank)
            torch.distributed.init_process_group(backend='cncl', init_method='env://')
    ...
    if args.device == 'gpu':
        ...
        train_loss_fn = train_loss_fn.to(ct.mlu_device())
        validate_loss_fn = nn.CrossEntropyLoss().to(ct.mlu_device())
    ...
    if ((args.distributed == False and args.hvd == -1) or (args.rank == 0)):
        _logger.info("###################print args below before trining###################")
        _logger.info(args)
        _logger.info("#####################################################################")
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if batch_idx == args.iters:
            _logger.info("just train %d iters." %(args.iters))
            break
        if not args.prefetcher:
            if args.device == 'gpu':
                input, target = input.cuda(), target.cuda()
            elif args.device == 'mlu':
                input, target = input.to(ct.mlu_device()), target.to(ct.mlu_device())
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        ...
        metric_collector.insert_metrics(net = args.model,
                                        accuracy = [top1_m.avg, top5_m.avg])
        if ((args.distributed == False and args.hvd == -1) or (args.rank == 0)):
            metric_collector.dump()
