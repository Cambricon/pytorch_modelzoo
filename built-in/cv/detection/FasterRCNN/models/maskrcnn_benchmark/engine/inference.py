...
def compute_on_dataset(model, data_loader, device, bbox_aug, timer=None,cfg=None):
    model.eval()
    results_dict = {}
...
def _accumulate_predictions_from_multiple_devs(predictions_per_dev):
    all_predictions = all_gather(predictions_per_dev)
...
def inference(
    ...
    total_timer.tic()
    predictions = compute_on_dataset(model, data_loader, device, bbox_aug, inference_timer,cfg)
    ...
    predictions = _accumulate_predictions_from_multiple_devs(predictions)
    if not is_main_process():
        return
