...
def test(data,
    ...
    if model is None:
        ...
        device = torch_utils.select_device(opt.device, batch_size=batch_size, device_id=opt.device_id)
        ...
        model = Model(opt.cfg)
        ckpt = torch.load(weights, map_location='cpu')['model']
        ...
        if opt.device == 'mlu' and opt.cnmix:
            import cnmix
            model, _ = cnmix.initialize(model, None, opt_level=opt.opt_level)
            if ckpt.get('cnmix') is not None:
    ...
    if save_json and map50 and len(jdict):
        ...
        metric_collector.insert_metrics(net="yolov5s", accuracy=map50)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='mlu', help='cpu, gpu, mlu device')
    ...
    parser.add_argument('--opt_level', type=str, default='O0', help='choose level of optimizer')
    ...
    if opt.task in ['val', 'test']:  # (default) run normally
        ...
        for weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt']:
            ...
            for i in x:  # img-size
                ...
                r, _, t = test(opt.data, weights, opt.batch_size, i, opt.conf_thres, opt.iou_thres, opt.save_json, eval_iters=opt.eval_iters)
