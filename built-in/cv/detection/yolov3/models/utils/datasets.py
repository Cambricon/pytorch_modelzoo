...
from utils.utils_train import xyxy2xywh, xywh2xyxy
help_url = 'https://github.com/ultralytics/yolov3/wiki/Train-Custom-Data'
...
class ListDataset(Dataset):
    def __init__(self, coco_path, list_path, img_size=416, normalize=False):
    ...
    def __getitem__(self, index):
        ...
        img_path = self.coco_path + '/' + img_path
        img = np.array(Image.open(img_path))
        ...
        if self.normalize:
            ...
            input_img = cv2.resize(input_img, (self.img_shape[0], self.img_shape[1]), interpolation = cv2.INTER_AREA)
        # Channels-first
        input_img = np.transpose(input_img, (2, 0, 1))
        # As pytorch tensor
        input_img = torch.from_numpy(input_img).float()
        ...
        if os.path.exists(label_path):
            ...
            labels[:, 3] *= w / float(padded_w)
            labels[:, 4] *= h / float(padded_h)
        # Fill matrix
...
def with_data_env(x):
    real_path = os.getenv("PYTORCH_TRAIN_DATASET")
    from_env = True if real_path is not None else False
    real_path = real_path + "/COCO2014"
    if from_env:
        x = real_path + "/images/" + x.split('/')[-2] + "/" + x.split('/')[-1]
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        with open(path, 'r') as f:
            self.img_files = [with_data_env(x) for x in f.read().splitlines()  # os-agnostic
