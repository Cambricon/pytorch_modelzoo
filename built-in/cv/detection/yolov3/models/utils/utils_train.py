...
try:
    ...
    # batched_nms avoid going through jit
except ImportError:
    print("train/test without MLU!")
# Set printoptions
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    ...
    box2 = box2.t().clone().requires_grad_(True) 
    ...
    # Intersection area
    inter = (torch.min(b1_x2, b2_x2) - torch.max(b1_x1, b2_x1)).clamp(min=0) * (torch.min(b1_y2, b2_y2) - torch.max(b1_y1, b2_y1)).clamp(min=0)
    # Union Area
...
def build_targets(model, targets):
    ...
    for i in model.module.yolo_layers if multi_mlu else model.yolo_layers:
        ...
        if c.shape[0]:  # if any targets
            ...
            assert c.max() < 80, 'Model accepts %g classes labeled from 0-%g, however you labelled a class %g. ' \
...
def print_model_biases(model):
    ...
    multi_mlu = type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2 = model(torch.Tensor(ims)).argmax(1)  # classifier prediction
...
def plot_images(imgs, targets, paths=None, fname='images.png'):
    ...
    # here plot images, and use numpy, so must put tensors on cpu
    imgs = imgs.cpu().numpy()
