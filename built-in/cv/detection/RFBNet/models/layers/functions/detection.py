...
class Detect(Function):
    ...
    def forward(self, predictions, prior):
        #print('Detect:forward') #not used in training
        """
        Args:
        ...
        if _USE_MLU and ct.is_mlu_tensor(loc_data):
            self.boxes = self.boxes.to(ct.mlu_device())
            self.scores = self.scores.to(ct.mlu_device())
        if _USE_GPU and loc_data.is_cuda:
        ...
        for i in range(num):
            ...
            # replace the normal slicing with the advanced index, now reverted since stride is sorted out in MLU
