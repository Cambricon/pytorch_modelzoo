...
def boxes_iou3d_gpu(boxes_a, boxes_b):
    ...
    # bev overlap
    if boxes_a.device.type == 'mlu':
        overlaps_bev = torch.ops.torch_mlu.boxes_overlap_bev(boxes_a.contiguous(), boxes_b.contiguous())
    else:
...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, **kwargs):
    ...
    if boxes.device.type == 'mlu':
        out = torch.ops.torch_mlu.nms3D(boxes, thresh).long()
        return order[out].contiguous(), None
    else:
