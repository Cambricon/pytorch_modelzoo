...
def all_gather(data):
    ...
    if not isinstance(data, torch.Tensor):
        ...
        tensor = torch.ByteTensor(storage).to("mlu") if dist.get_backend() == 'cncl' else \
    ...
    if dist.get_backend() == 'cncl':
        local_size = torch.LongTensor([tensor.numel()]).to("mlu")
        size_list = [torch.LongTensor([0]).to("mlu") for _ in range(world_size)]
    else:
    ...
    if dist.get_backend() == 'cncl':
        for _ in size_list:
            tensor_list.append(torch.FloatTensor(size=(max_size,)).mlu().to(tensor_type))
        if local_size != max_size:
            padding = torch.FloatTensor(size=(max_size - local_size,)).mlu().to(tensor_type)
