...
def init_dist_pytorch(tcp_port, local_rank, backend='nccl'):
    ...
    if backend == 'cncl':
        num_devs = torch.mlu.device_count()
        torch.mlu.set_device(local_rank % num_devs)
    else:
        num_devs = torch.cuda.device_count()
        torch.cuda.set_device(local_rank % num_devs)
...
def get_dist_info(return_gpu_per_machine=False):
    ...
    if return_gpu_per_machine:
        if _USE_MLU:
            dev_per_machine = torch.mlu.device_count()
        else:
            dev_per_machine = torch.cuda.device_count()
        return rank, world_size, dev_per_machine
