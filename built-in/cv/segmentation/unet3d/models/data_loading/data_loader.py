...
def get_data_loaders(flags, num_shards):
    ...
    if(flags.world_size>1):
        train_sampler = DistributedSampler(train_dataset, num_replicas=flags.world_size, rank=flags.local_rank, seed=flags.seed) if num_shards > 1 else None
        val_sampler = DistributedSampler(val_dataset, num_replicas=flags.world_size, rank=flags.local_rank, seed=flags.seed) if num_shards > 1 else None
    else:
