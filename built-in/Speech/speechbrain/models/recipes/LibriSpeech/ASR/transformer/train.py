...
class ASR(sb.core.Brain):
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID and sb.utils.distributed.if_main_process():
            ...
            self.avg_collector.insert_metrics(net="conformer",
                                              accuracy = stage_stats["ACC"],
                                              epoch = epoch)
    ...
    def on_fit_start(self):
        ...
        super().on_fit_start()
        self.avg_collector = MetricCollector(enable_only_avglog=True)
...
if __name__ == "__main__":
    ...
    asr_brain.metric_collector.insert_metrics(
                net = "conformer",
                batch_size = asr_brain.train_batch_size,
                precision = "fp32",
                cards = int(os.environ["WORLD_SIZE"]) if asr_brain.distributed_launch else 1,
                DPF_mode = "ddp" if asr_brain.distributed_launch else 'single')

    if (not asr_brain.distributed_launch or int(os.environ["LOCAL_RANK"]) == 0):
        asr_brain.metric_collector.dump()
