...
def ddp_init_group(run_opts):
    ...
    if run_opts["distributed_launch"]:
        if "local_rank" not in run_opts:
            pass
          # Remove check of local_rank, MLU use distributed.run instead.
          ...
          #  )
            ...
            if run_opts["device"] == 'cuda' and run_opts["local_rank"] + 1 > torch.cuda.device_count():
                raise ValueError(
                    "Killing process " + str() + "\n"
                    "Not enough GPUs available!"
                )
        if "LOCAL_RANK" in os.environ is None or os.environ["LOCAL_RANK"] == "":
            raise ValueError(
                "To use DDP backend, start your script with:\n\t"
                "python -m torch.distributed.launch [args]\n\t"
                "experiment.py hyperparams.yaml --distributed_launch=True "
        ...
        if "mlu" in run_opts['device']:
            import torch_mlu
            torch.mlu.set_device(int(os.environ["LOCAL_RANK"]))
        rank = int(os.environ["LOCAL_RANK"])

        if run_opts["distributed_backend"] == "nccl":
            if not torch.distributed.is_nccl_available():
                raise ValueError("NCCL is not supported in your machine.")
        elif run_opts["distributed_backend"] == "cncl":
            # MLU has no cncl available check at yet.
            pass
