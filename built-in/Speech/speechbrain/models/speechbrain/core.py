...
def parse_arguments(arg_list):
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of batches to run in train.",
    )
    parser.add_argument(
        "--valid_batches",
        type=int,
        default=-1,
        help="Number of batches to run in valid.",
    )
    parser.add_argument(
        "--eval_batches",
        type=int,
        default=-1,
        help="Number of batches to run in eval.",
    )
    parser.add_argument(
        "--debug_epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Flag of only one epoch daily test."
    )
    parser.add_argument(
        "--log_config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="cncl",
        help="One of {cncl, nccl, gloo, mpi}",
    )
    parser.add_argument(
    ...
    # Checking that DataParallel use the right number of GPU or MLU
    if run_opts["data_parallel_backend"]:
        if torch.cuda.device_count() == 0:
            try:
                if torch.mlu.device_count() == 0:
                    raise ValueError("You must have at least 1 GPU or MLU.")
            except Exception as e:
                print("import torch_mlu failed!")
                raise ValueError("You must have at least 1 GPU or MLU.")
    ...
    if local_rank is not None and "mlu" in run_opts["device"]:
        run_opts["device"] = run_opts["device"][:-1] + str(local_rank)
...
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def __init__(  # noqa: C901
        ...
        if "mlu" in self.device and run_opts['distributed_launch'] is True:
            torch.mlu.set_device(int(os.getenv('LOCAL_RANK',0)))
    ...
    def fit(
        ...
        * ``update_average()``
        ...
        if not (
            ...
            self.train_batch_size = train_loader_kwargs["batch_size"]
        if valid_set is not None and not (
            ...
            self.valid_batch_size = valid_loader_kwargs["batch_size"]
        ...
        for epoch in epoch_counter:
            ...
            with tqdm(
                ...
                for step, batch in enumerate(t):
                    self.step += 1
                    # Only train a few batches if train_batches if specified
                    if step == self.train_batches:
                        break
                    if batch.batchsize != self.train_batch_size:
                        break
            ...
            if valid_set is not None:
                ...
                with torch.no_grad():
                    for step, batch in enumerate(tqdm(
                        ...
                        # Only valid a few batches if valid_batches if specified
                        if step == self.valid_batches:
                            break
                        if batch.batchsize != self.valid_batch_size:
                            break
                        self.step += 1
            ...
            # Daily mode only runs to max epochs due to the checkpoint.
            if self.one_epoch:
                break
