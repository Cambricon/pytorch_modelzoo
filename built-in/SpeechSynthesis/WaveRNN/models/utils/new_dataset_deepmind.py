...
def get_deepmind_datasets(n_seed, path: Path, batch_size, seq_len, distributed_run):
    ...
    train_set = DataLoader(train_dataset,
                           ...
                           shuffle=shuffle,
                           collate_fn=lambda x: collate_vocoder(x, seq_len),
                           batch_size=1,
                           num_workers=8,
                           pin_memory=True)
...
def collate_vocoder(batch, seq_len):
    coarse = batch[0][0]
    fine = batch[0][1]
    rand_idx = np.random.randint(0, coarse.shape[1] - seq_len - 1)
    x_coarse = coarse[:, rand_idx:rand_idx + seq_len] / 127.5 - 1.
    x_fine = fine[:, rand_idx:rand_idx + seq_len] / 127.5 - 1.
    y_coarse = coarse[:, rand_idx + 1:rand_idx + seq_len + 1]
    y_fine = fine[:, rand_idx + 1: rand_idx + seq_len + 1]

    x_coarse = torch.tensor(x_coarse)
    x_fine = torch.tensor(x_fine)
    y_coarse = torch.tensor(y_coarse)
    y_fine = torch.tensor(y_fine)

    return x_coarse, x_fine, y_coarse, y_fine
