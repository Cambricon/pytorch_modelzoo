...
class T5Attention(nn.Module):
    ...
    def _relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        ...
        if bidirectional:
            ...
            relative_position = relative_position.to(torch.int32)
        else:
            relative_position = -torch.min(relative_position, torch.zeros_like(relative_position)).float()
            relative_position = relative_position.to(torch.int32)
    ...
    def compute_bias(self, query_length, key_length):
        ...
        context_position = torch.arange(
            query_length, dtype=torch.int32, device=self.relative_attention_bias.weight.device
        )[:, None]
        memory_position = torch.arange(
            key_length, dtype=torch.int32, device=self.relative_attention_bias.weight.device
        )[None, :]
    ...
    def forward(
        ...
        if self.is_decoder and encoder_attention_mask is None and encoder_hidden_states is not None:
            ...
            encoder_attention_mask = torch.ones(
                batch_size, encoder_seq_length, device=inputs_embeds.device, dtype=torch.int32
            )
