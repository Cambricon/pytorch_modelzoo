...
def is_main_process_with_hvd(args):
    if args.hvd != -1:
       return args.hvd_rank == 0
    else:
...
def main():
    ...
    parser.add_argument("--eval_iters", default=-1.0, type=float,
                        help="Total number of training steps to perform.")
    ...
    parser.add_argument("--hvd",
                        default=-1,
                        type=int,
                        help="how many cards used in horovod")
    ...
    if args.hvd == -1 and args.local_rank == -1:
    ...
    if is_main_process_with_hvd(args):
    ...
    print("device: {} n_device: {}, distributed training: {}, horovod training: {} ,cnmix training: {}, opt_level: {}".format(
                                device, n_device, bool(args.local_rank != -1 and args.hvd == -1), bool(args.hvd != -1) ,args.cnmix, args.opt_level))
    ...
    if not os.path.exists(args.output_dir) and is_main_process_with_hvd(args):
        os.makedirs(args.output_dir)

    tokenizer = BertTokenizer(args.vocab_file, do_lower_case=args.do_lower_case) # for bert large
    ...
    if args.do_train:
        ...
        if args.local_rank != -1 and args.hvd == -1:
            num_train_optimization_steps = num_train_optimization_steps // torch.distributed.get_world_size()
        elif args.local_rank != -1 and args.hvd != -1:
    ...
    if args.do_predict:
        ...
        num_predict_steps = int(
            len(eval_examples) / args.predict_batch_size )
    ...
    if args.do_train:
        if False: #args.fp16:  reserved for probable apex support in script
            try:
        ...
        if args.cnmix:
            ...
            if args.loss_scale == 0:
                ...
                model, optimizer = cnmix.initialize(model, optimizer, opt_level=args.opt_level, loss_scale=args.loss_scale)
            if 'cnmix' in ckpt:
                cnmix.load_state_dict(ckpt['cnmix'])

    if args.local_rank != -1 and args.hvd == -1:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank], bucket_cap_mb=100, find_unused_parameters=True)
    elif n_device > 1:
    ...
    if args.do_train:
        ...
        try:
            ...
            if not args.skip_cache and is_main_process_with_hvd(args):
        ...
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size * n_device)

        if args.cnmix:
            cnmix.core.cnmix_set_amp_quantify_params('all', {'batch_size': args.train_batch_size * n_device, 'data_num': len(train_dataloader) * args.train_batch_size * n_device})
        ...
        for epoch in range(int(args.num_train_epochs)):
            train_iter = tqdm(train_dataloader, desc="Iteration", disable=args.disable_progress_bar) if is_main_process_with_hvd(args) else train_dataloader
            ...
            DPF_mode = "ddp" if  args.local_rank != -1 else "hvd" if args.hvd != -1 else "single")
            if ((args.local_rank == -1 and args.hvd == -1) or (args.local_rank == 0)):
                metric_collector.dump()
        time_to_train = time.time() - train_start

    if args.do_train and is_main_process_with_hvd(args) and not args.skip_checkpoint:
        # Save a trained model and the associated configuration
        model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
        output_model_file = os.path.join(args.output_dir, modeling.WEIGHTS_NAME)
        save_params={"model":model_to_save.state_dict()}
        if args.cnmix:
           save_params["cnmix"]=cnmix.state_dict()
        torch.save(save_params, output_model_file)
    ...
    if args.do_predict and (args.local_rank == -1 or is_main_process_with_hvd(args)):
        if not args.do_train and args.cnmix:
            model.half()
        ...
        for step, (input_ids, input_mask, segment_ids, example_indices) in enumerate(tqdm(eval_dataloader, desc="Evaluating", disable=args.disable_progress_bar)):
        ...
        if args.do_eval and is_main_process_with_hvd(args):
            import sys
            import subprocess
        ...
        if args.max_steps == -1:
            ...
            dllogger.log(step=tuple(), data={"e2e_train_time": time_to_train,
                                              ...
                                              * args.max_steps * gpu_count / time_to_train,
                                              "final_loss": final_loss})
    if args.do_predict and is_main_process_with_hvd(args):
    ...
    if args.do_eval and is_main_process_with_hvd(args):
        ...
        metric_collector_avg_log = MetricCollector(enable_only_avglog=True)
        metric_collector_avg_log.insert_metrics(net = "BERT",
                                        accuracy = [exact_match, f1])
        if args.local_rank <= 0:
            metric_collector_avg_log.dump()
