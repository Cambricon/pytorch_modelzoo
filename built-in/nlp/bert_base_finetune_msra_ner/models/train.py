...
parser.add_argument('--multi_device', default=False, action='store_true', help="Whether to use multiple Devices if available")
...
if __name__ == '__main__':
    ...
    if args.device == 'mlu':
        ...
        params.n_device = 1
    params.multi_device = args.multi_device

    # Set the random seed for reproducible experiments
    random.seed(args.seed)
    torch.manual_seed(args.seed)
    if args.device == 'gpu' and params.n_device > 0:
    ...
    # DP for gpu, mlu not support DP
    if args.device=='gpu' and params.n_device > 1 and args.multi_device:
