...
function print_log() {
  echo -e "\033[31m ERROR: Please execute the command: cd pytorch_models/tools/mlu_performance_check; sudo bash set_mlu_perf.sh\033[0m"
}
...
function checkNeuwareVersion() {
  pushd $CATCH_HOME/script/release/
    ...
    # dependency2.txt is generated by catch/script/release/json_parser.py ,only used for pytorch models to check neuware version.
    neuware_lib_version=$(awk -F ':' '{if(($1 !~ "cntoolkit")&&($1 !~ "cnplugin")&&($1 !~ "cnlight")&&($1 !~ "magicmind")) print $1, $2, $3}' dependency2.txt | awk '{printf "lib%s.so.%s\n", $1, $3}' | awk -F '-' '{printf "%s\n", $1}')
  popd
